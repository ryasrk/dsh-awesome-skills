...
export class SkillIndex {
    ...
    hit(i, score, pinned = false) {
        const m = this.meta[i];
        const out = { name: m.name, path: m.path || m.name, score: +score.toFixed(4), description: this.clip(m.description, 300) };
        if (pinned)
            out.pinned = true;
        return out;
    }
