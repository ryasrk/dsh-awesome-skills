...
const stem = (w) => {
    if (SURVIVE.has(w))
        return w;
    if (w.length >= 9 && w.endsWith('ment') && w.length - 4 >= 5)
        return w.slice(0, -4);
    if (w.length >= 7 && w.endsWith('ing') && w.length - 3 >= 4)
        return w.slice(0, -3);
    if (w.length >= 6 && S2_ENDS.has(w.slice(-2)) && w.length - 2 >= 4)
        return w.slice(0, -2);
    if (w.length >= 5 && w.endsWith('s') && w.length - 1 >= 4)
        return w.slice(0, -1);
    return w;
};
const toks = (s) => String(s).toLowerCase().replace(/[^a-z0-9]+/g, ' ').split(' ').filter(w => w.length > 2 && !STOP.has(w)).map(stem);
...
export class SkillIndex {
        ...
        const scored = order.map(i => {
            ...
            if (qBigrams.length > 0) {
                ...
                for (const b of qBigrams) {
                    if (boost >= 0.3)
                        break;
                    if (new RegExp('\\b' + b.replace(/[^a-z0-9 ]/g, '') + '\\b').test(docText))
                        boost += 0.15;
                }
            }
    ...
    visibleIndices() {
        ...
        for (const [path, idx] of this.byPath) {
            if (allow !== undefined && !allow.has(path))
                continue;
            if (bl.has(path))
                continue;
            out.add(idx);
