...
process.stdin.on('end', async () => {
  const t0 = Date.now()
  try {
    const req = JSON.parse(input) as { query?: unknown; k?: unknown }
    ...
    process.stdout.write(
      JSON.stringify({ ok: true, ms: Date.now() - t0, count: index.count(), results: hits }) + '\n',
    )
  } catch (error) {
    process.stdout.write(
      JSON.stringify({ ok: false, ms: Date.now() - t0, error: error instanceof Error ? error.message : String(error) }) + '\n',
    )
  }
})
