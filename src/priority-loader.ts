...
export interface InjectedMessage {
  ...
  source?: { kind: 'priority-skills'; digest: string }
}
 ...
 */
function renderPrioritySkill(name: string, content: string): string {
  return [
 ...
 */
export function installPriorityLoader(
  ctx: {
    on?: unknown
    logger?: { warn(message: string): void }
  },
  corpusDir: string,
  getKnobs: () => PriorityKnobs,
  ...
  const lastInjection = (
    ...
    for (let index = events.length - 1; index >= 0; index -= 1) {
      ...
      if (source?.kind !== 'priority-skills') continue
      return {
