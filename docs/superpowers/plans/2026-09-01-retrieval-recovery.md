...
## Outcome 2 (recorded 2026-09-02)

Round 2 grew the canonical set 50 → 100 labels (frozen-50 preserved; labels100.json + bench100.mjs with the same gold-match rule) and re-ran the lever ladder with a pre-registered conjunctive gate: a lever ships only if R@1 ≥ 71% AND R@3 ≥ 92% on the 100-label bench. Bar for the final numbers: R@1 ≥ 70% / R@3 ≥ 90%.

- **Final shipped numbers** (stems-only code at `a3bec6b`): **R@1 76/100 (76%), R@3 92/100 (92%), median 0.581** — vs base-100 pre-stem standing 71/92/0.565. Bar met on both metrics. Band rule: median 0.581 ≥ 0.55 AND R@3 92% ≥ 80% → `keep 0.7 / 0.4` (shipped bands unchanged).
- **Lever 1 — stems-only probe (guarded suffix stemming, no bigram boost): KEEP** (`a3bec6b` on top of `f0fe089`). +5 R@1 labels over base-100 with R@3 held exactly: functions→function, refactoring→refactor, traces→trac↔tracing, fonts→font, resources→resourc.
